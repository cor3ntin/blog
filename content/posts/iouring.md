...
date: 2020-01-28T14:43:18+01:00
draft: true
---
...
Last time we discussed two base concepts:
...
Threads are less expensive than processes but they still take time to create: Avoid having one thread per task if you have thousands of tasks.
...
This can happen either on a single thread (some tasks would be enqueud before the program starts waiting for file descriptors event) or happen across multiple threads, in which case we need to syncronize file registration. More on that later.
...
* Read the data (in a non plocking fashion) (1 syscall)
...
* Register the file descriptor along with a set of buffers fo fill
* Run some other task
...
This reduces the number of syscalls and lets us only resume tasks when the desired I/O have been fulfilled.
Internally the kernel may spawn its own pool of working threads to perform the I/O operations,
nothing being ever truly free.
...
In fact `AIO` on linux is implemented in user space - but a similar kernel APIs `io_submit` can be used instead. In any case, this APIs is designed to handle file i/o and it is either not possible or not recommended to use it for sockets as `epoll` would perform better in all cases.
...
Maybe this explains why we have both **ASIO** and **AFIO** as different projects with different interfaces, instead of some general asynchronous system like [`libuv`](https://libuv.org) or [Tokio](https://github.com/tokio-rs/tokio).
...
but has since then improved support for sockets and might morph into a general asynchronous system call interface.
...
completion queue even as the kernel reads to it.
...
Moreover, the submission queue can only be accessed by a single thread concurrently.

Once work has been added to the ring, a single system `io_uring_enter` can be used
...
which handles the very low-level user-space ruing management for us.
...
In practice, we would use a non-allocating linked list for the queue,
and try to make it lock-free.
...
`io_uring` happens to have built-in timout support. Other scheduler may use [`timerfd`](http://man7.org/linux/man-pages/man2/timerfd_create.2.html) or [`CreateThreadpoolTimer`](https://docs.microsoft.com/en-us/windows/win32/api/threadpoolapiset/nf-threadpoolapiset-createthreadpooltimer?) on windows.
...
It is more likely than instead we get few io objects such as files and sockets
...
Under some implementations, there is a thread started for every `std::async` call (one of the many reason why `std::async` is terrible).
...
There have some libraries that can be used on linux such as libdispatch or libuv, or implementers
can cook up something for scratch.
...
But before we can talk about handling cancellation let's stop about emitting a cancellation
request.
...
`stop_source` can create tokens, `stop_token` has a `stop_requested` methods that returns
...
Let's talk about a Golang.

## Gorroutines[^5] are not Goroutines

Goroutines, a feature of the Go programming language, in addition of being stackful, are very
different from C++ coroutines in that they are both a resumption mechanism and a scheduling mechanism.
Go provides you with a baked-in i/o and coroutines scheduler which will handle on behalf of the
program interrupting a goroutiners when it performs an i/o, try to acquire a lock or any other blocking operation.
...
against the "Don't pay for what you don't use" mantra. In fact, it would make C++ unusable in many environments.
...
[^10]: [`std::jthread`](https://en.cppreference.com/w/cpp/thread/jthread) is now the recommended way to start a thread in C++ - I think it would be
fair to consider `std::thread` deprecated, and maybe reflect on how we got into this infortunate situation.
[^11]: _Someone_ should write a blog post about that
