...
You might also have noticed that the paper is title "**span: bounds-safe views**".
...
But fundamentally, the only difference between a vector and a string is that which is convey by the programmers' intent.
...
(Pardon my Chinese). At best, you can hope that if you don't mutate it in anyway or look at it funny, it might still look okay by the time you display it somewhere.
...
> In a somewhat analogous sitation, LLVM defined an ArrayRef class in Feb 2011, and didn't find a need for the matching MutableArrayRef until Jan 2012.
...
So having a type being `const` by default, while more appealing to our modern, wiser sensibilities may not be that great : there is no way to opt-out from from `basic_string_view` constness.
Since `mutable` always is the default, the language do not provide a way to construct a `basic_string_view<mutable char>`.
...
Instead of a representing a range with pair of iterators whose type depends on the underlying container,
...
So one one hand, `span` is a range, on the other hand, it does not play well with ranges.
...
This behave conceptually like the standard `std::span` and yet it is easier to understand and reason about.
...
`string_view` being a further refinment on span, it's clear that the committee started with the more specialized solution
and is progressing to the general cases, leaving odd inconsistancies in its wake.
...
I would go so far as to say than `span` (and `view`, same thing) breaks C++.
...
This types properties are statically checked at compile time, however if a type definition do not match its behaviour, the algorithm will compile
...
`span` looks like a castor and bites like a snake, but is actually a duck-billed platypus, a monstrous hybrid that foils every attempt at
classification.
...
By its dual nature, its api is easy to misused and its humble appearance make it look like an innocent container-like thing rather than the deadly trap it is.
 ...
 * It's a container-like object whose assignment do not actually change the underlying data.
...
A lot can be said to the importance of naming things properly and as far as `span` is concerned, there are more than a few names wrong.

Let's unpack

## span::operator==()

There are whole fields of mathematics dedicated to describe how things are "equal" or comparable.
...
The only way to avoid a blood bath is to therefore **remove completely all comparison operators**.
If you can't compare them, you can't compare them wrongly.

Unfortunately, if a type isn't comparable, the `stl` kinda stop working - the type stop being `Regular` and concretely sort and search algorithms will not work.
...
Elements comparison would be done through `std::equal`.
...
`array` carries that it is a pointer to a contiguous memory segment, because that's what array are in the C memory model.
...
A couple of papers were publishing, releving more issues with span
* [Its size is, for some reason, signed] (https://wg21.link/p1089)
* [Its API has some inconsistencies] (https://wg21.link/p1024)

## Changing people ?
Some believe that we should teach people that playtipus are ducks because that sure would be convenient.
...
sounds a bit unreasonable to be. At best it takes decades, and by the time the collective knowlegde and widsom starts shifting, the
...
And I wonder if - as much fun as it may be to see them jumping over the canopee - we actually have a need for non mutable flying squirrels.
